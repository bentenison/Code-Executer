...
class TestCase:
  ...
  methods: {
    ...
    truncatedTitle(fullTitle) {
      return fullTitle.length > this.maxLength
        ? fullTitle.substring(0, this.maxLength) + "..."
        : fullTitle;
    },
    switchTheme(theme) {
      this.theme = theme;
    ...
    formatResults(dataArray) {
      const listItems = dataArray
        .map((item) => {
          const inputs = item.input.join(", ");
          const output = item.expectedOutput;
          return `<li>
                            <span style="color: blue;">Inputs: [${inputs}]</span> 
                            <span style="color: green;">=> Output: ${output}</span>
                        </li>`;
        })
        .join(""); // Join all formatted strings without newline

      return `<ul class="result-list">${listItems}</ul>`; // Return an unordered list
    },
    submitQuestion() {
      this.mainStore.togglePageBlock();
      this.runBtnLoading = true;
      ...
      let payload = {
        ...
        file_extension: this.challengeStore.selectedLanguage.file_extension,
        is_challenge: true,
        challenge_id: this.challengeStore.challenges.challenge_id,
      };
      // console.log("langIDDDDDDDDDDD",this.langId);
      this.editorStore
        .submitCode(payload)
        .then((res) => {
          ...
          try {
            ...
            if (Array.isArray(parsed)) {
              const formatted = this.formatResults(parsed);
              // console.log("output", formatted);
              this.emitter.emit("showMessage", formatted);
            }
          } catch (e) {
            // console.log("execeptionn", e);
            this.emitter.emit("showMessage", res.data.output);
          }
          this.runBtnLoading = false;
          ...
          if (index !== -1) {
            // this.challengeStore.$patch(
            (this.challengeStore.challenges.questions[index].isCorrect =
              res.data.isCorrect),
              (this.challengeStore.challenges.questions[index].is_completed =
                true);
            // );
          }
          const idx = this.challengeStore.challengeQuestions.findIndex(
            (item) => item.id === payload.question_id
          );
          console.log("idx", idx);

          if (idx !== -1) {
            // this.challengeStore.$patch(
            (this.challengeStore.challengeQuestions[idx].isCorrect =
              res.data.isCorrect),
              (this.challengeStore.challengeQuestions[idx].is_completed = true);
            // );
          }
          this.mainStore.togglePageBlock();
        })
    ...
    setupEditor() {
      ...
      this.challengeStore
        ...
        .then((res) => {
          ...
          if (index !== -1) {
            this.currChallangeQuest =
              this.challengeStore.challenges.questions[index];
          }
